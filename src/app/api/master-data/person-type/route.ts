...
export async function GET(_: NextRequest, { }: ParamSegment) {
    ...
    try {
        ...
        const result = await apiService.getMasterDataApi().masterDataPersonTypeGet({ employeeID: employeeId });
        return NextResponse.json(result);
    }
    catch (err: any) {
        return NextResponse.json({ message: `[ERROR] /api/master-data/person-type [${employeeId}]`, err }, { status: 500 });
    }
}
