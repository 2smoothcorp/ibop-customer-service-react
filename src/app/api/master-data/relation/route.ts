...
export async function GET(_: NextRequest, { }: ParamSegment) {
    ...
    try {
        ...
        const result = await apiService.getMasterDataApi().masterDataRelationGet();
        return NextResponse.json(result);
    }
    catch (err: any) {
        return NextResponse.json({ message: `[ERROR] /api/master-data/relation [${employeeId}]`, err }, { status: 500 });
    }
}
