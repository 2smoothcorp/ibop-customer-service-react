...
export async function GET(_: NextRequest, { }: ParamSegment) {
    ...
    try {
        ...
        const result = await apiService.getMasterDataApi().masterDataTitlesGet({ employeeID: employeeId });
        return NextResponse.json(result);
    }
    catch (err: any) {
        return NextResponse.json({ message: `[ERROR] /api/master-data/titles [${employeeId}]`, err }, { status: 500 });
    }
}
